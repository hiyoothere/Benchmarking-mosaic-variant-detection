...
b= 7
tool_list = ['3.MF', '4.MT','7.HC20','12.HC200']
for tool in tool_list:
	dic_vaf_ans = {}
        dic_vaf_call = {}
	out = open('single_Perf_by_VAF.hc.ind.D3.txt','a')
	filelist = glob.glob("/data/project/MRS/4.Analysis_Mosaic/" + tool + "/8.IND_DS/tp/*D3*TP.vcf")
	l_cnt = []
	for file in filelist:
		...
		print 'l_vars: ' , l_vars
                for i in l_vars:
               		v = i.split(':')[0]
                        type = i.split(':')[1]
                        Cnt_Ans = Expect_VAF.Cnt(v,type, 'hc', 'ind')
	...
	for i in range(len(l_cnt)):
		for vaf in l_cnt[i]:
			...
			if vaf not in dic_vaf_call: 
				...
				dic_vaf_call[vaf] += call
	dic_vaf_ans = {}
	dic_vaf_ans = __Dic_VAF_ANS.get(dic_vaf_ans, 'ind')
	...
	for i in l_key:
        ...
        fpfilelist = glob.glob("/data/project/MRS/4.Analysis_Mosaic/" + tool + "/8.IND_DS/mx/*D3*FP.vcf")
	fp2 =  glob.glob("/data/project/MRS/4.Analysis_Mosaic/" + tool + "/8.IND_DS/tp/*D3*FP.vcf")
        ...
        l_graph = [1,2,4,5,9.6, 15, 22] #1, 2, 4, 5, 9.6, 19.2, 56
        idx = 0
        for i in l_key:
#                print tool, i, f_ans[i], f_call[i], f_callfp[i]
                ...
                if f_call[i] == 0:
                        ...
                        if f_callfp[i] == 0:
                                ...
                                cof = float((f_call[i] + f_callfp[i]))/(f_call[i]+(81.63*f_callfp[i]))
