...
import sys
import os
Tool = sys.argv[1] #Method

DIR="/data/project/MRS/4.Analysis_Mosaic/" + Tool + "/1.DF_A/SetA/" # output path of parsed variant calls from single sample methods, both SetA and SetB are applied seperately
OUTDIR="/data/project/MRS/4.Analysis_Mosaic/" + Tool + "/1.a.DF_PA/SetA/" # Intersection output path
l_TF = ["TP", "FP"]
...
def make_analysis(case,cont):
        CASE = open(DIR + Tool.split('.')[1]+"." + Case + "_"+TF+".vcf","rb") # Case samples are M3 type samples (18)
        CONT = open(DIR + Tool.split('.')[1] +"." + Control +"_"+TF+".vcf","rb") # Control samples are M1 and M2 type samples (21, 9 M1 and 12 M2)
