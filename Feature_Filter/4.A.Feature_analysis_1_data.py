...
_type = ['SetB']
for typ in _type:
    _dir = '/data/project/MRS/4.Analysis_Mosaic/4.MT/1.DF_A/' + _type # parsed form of output calls from methods
    ...
    ##########################
    fo_MT = open('/data/project/MRS/5.Combined_Analysis/feature_analysis/MT_'+typ+'TPFP_feature.txt','w') # output file
    fo_MT.writelines("ID"+'\t'+"TP_FP"+'\t'+"GERMQ"+'\t'+"MMQ_ref"+'\t'+"MMQ_alt"+'\t'+"MBQ_ref"+'\t'+
                     "MBQ_alt"+'\t'+"MFRL_ref"+'\t'+"MFRL_alt"+'\t'+"ECNT"+'\t'+"POPAF"+'\t'+"TLOD"+'\t'+
                     "AF"+'\t'+"MPOS"+'\n') # Gather features of tools (MT2)
    
    for vcf in _TP: #True positives
    ...
    for vcf in _FP: #False positives
