...
weight = float(sys.argv[2]) #for precision recalibration (snv: 252.75, indel: 81.63)
out = open("/data/project/MRS/5.Combined_Analysis/Filter/Fil_ana." + tool + ".txt", "w") # output
types = ['SetA', 'SetB']
...
for type in types:
	...
	c_fn = 0
	c_tn = 0
	ori = open("/data/project/MRS/5.Combined_Analysis/Filter/" + tool + "." + type + ".summary.txt", "r") # output of 2.B.Variant_snv_performance.2.auprc.py
	for line in ori:
		s = line.split('\t')
		tp = int(s[2])
		fp = int(s[3])
	...
	dic_fil_fp = {}
	l_fil = []
	fs_TP = glob.glob("/data/project/MRS/4.Analysis_Mosaic/"+tool+"/6.FIL/"+type+"/*TP.snv.vcf") #variant call files without filter applied, only annotated
	for file in fs_TP:
		f = open(file, 'r')
		for line in f:
			s = line.split('\t')
