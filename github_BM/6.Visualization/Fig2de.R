...
data <- read.csv("/Users/tinkertwerk/Desktop/Results/Figures/Figure2/nA.single/single_Perf_by_VAF.hc.snv.txt", header=FALSE, sep='\t')
...
sen <- ggplot(data, aes(x = V11, y= V6, group = V1)) +
  ...
  scale_x_continuous(breaks = c(1, 3, 7.5, 9.6, 20, 35),label=c('< 1%','< 3%','< 7.5%','< 9.6%','< 25%', '> 25%')) +
  theme_bw() +
  scale_colour_manual(values = c('#BF4141','#F2780C','#F2AE2E','#491F73','#A0A603','#025959')) +
  scale_shape_manual(values = c(19, 19, 19, 17, 15, 15)) +
...
ggarrange(sen, pre, fscore, ncol = 3) %>%
  ggexport(filename = '/Users/tinkertwerk/Desktop/2.Benchmark/00.ForRevision/Fig/Fig2/snv.pdf')
