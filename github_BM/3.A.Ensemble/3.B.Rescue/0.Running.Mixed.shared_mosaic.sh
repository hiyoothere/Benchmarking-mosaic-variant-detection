...
script=$HOME/jisoo_backup/4.BM_Rescue
depth=$HOME/jisoo_backup/3.BM_MSM/depth
...
for idx_DP in ${!DP_ID[@]};do
	...
	if [[ $DP == *"ND"* ]];then
		...
		DEPTH=$depth/depth.5.95.txt ### NEEDS TO BE transplanted
	else
		CASE_BAM=$(ls $DSPath  | egrep "${DP}" | egrep ".bam" | egrep -v "bai" |  egrep  $M_CASE )
		path_tag_arr=("3.DS_A" "8.IND_DS")
		DEPTH=$depth/depth.5.95.${DP}.txt ### NEEDS TO BE transplanted
