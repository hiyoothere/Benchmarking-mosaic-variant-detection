...
# for MT call MF raw , alt softclip
# Call set
...
for typ in _type:
    ...
    MF_TP_count = 0
    MF_FP_count = 0
    fo_MF=open('/data/project/MRS/5.Combined_Analysis/combination/output/call_MT_raw_MF_'+typ+'_comparison.txt','w')
    ...
    fo_MF_percent=open('/data/project/MRS/5.Combined_Analysis/combination/output/call_MT_raw_MF_'+typ+'_comparison_percent.txt','w')
...
for typ in _type:
    ...
    fo_data = open(_dir+'/result/alt_softclip_'+typ+'_only_clip.txt','w')
    f_data = open(_dir+'/'+'call_MT_raw_MF_'+typ+'_comparison.txt','r')
    ...
    for m in range(1,len(line_data)):
        ...
        if lis_data[1] == 'TP':
            if float(lis_data[9]) >= 0.5: # filtered
                TP_filtered += 1
            else:
                TP_passed += 1
        elif lis_data[1] == 'FP':
