...
type=sys.argv[1] # Method
outpath=sys.argv[2] #Output path
dic_outname = { "MT2":"*PASS.snps.vcf" } # Method and its output suffix
turn=[ 'SetA', 'SetB']
for each_turn in turn:
	...
	for file in filelist:
		...
		l_nc = []
                dic_nc_ann = {}
		if each_turn == 'Setb':
			nc = open("/data/project/MRS/0.Genotype/ctrl/nc/4.hc_rp/hc/germline.nc.snv.vcf" , 'r') #Germline negative control
			for line in nc:
	                        if line[0] != '#':
        	                        s = line.split("\t")
                	                idx = get_id.gatk(line)
			...
			nc = open("/data/project/MRS/0.Genotype/ctrl/nc/4.hc_rp/hc/refhom.nc.vcf" , 'r') #Non-variant negative control
			for line in nc:
	                        if line[0] != '#':
        	                        s = line.split("\t")
                        	        CHR = s[0]
