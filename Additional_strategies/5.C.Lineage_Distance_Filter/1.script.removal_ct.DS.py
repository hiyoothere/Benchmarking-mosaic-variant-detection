...
combination = list(itertools.product(*M))

out = open(outpath + "/MT.D2." + tag + ".txt", 'w')
##cols = ["SAMPLES", "M1M2M3_TP", "M1M2M3_FP", "M1M2_TP","M1M2_FP", "M1M3_TP","M1M3_FP", "M1_TP", "M1_FP", "M1_total_FN", "TP_vrem_per", "FP_rem_per"]
...
for c in combination:
	...
	FN_ct = FN[c[0]]
	for s in c:
		TP = glob.glob(DIR + "/tp/" + tool + ".D2." + s + "_TP*") ##### TP from transplanted
		FP_mx = glob.glob(DIR + "/mx/" + tool + ".D2." + s + "_FP*") ##### artifact FP 
                FP_tp = glob.glob(DIR + "/tp/" + tool + ".D2." + s + "_FP*") ##### germline FP 
                temp = TP + FP_mx + FP_tp
		print temp
		for t in temp:
