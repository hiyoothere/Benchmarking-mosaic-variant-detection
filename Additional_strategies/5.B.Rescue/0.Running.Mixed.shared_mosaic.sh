...
# 5. data di
NDPath=$DIR/1.Mixed/3.aligned_Mosaic/bwa_mem/1.whole/ # bam file directory of 1100x
DSPath=$DIR/1.Mixed/5.downsample_Mosaic # bam file directory of 125x, 25x, and 500x
...
DP_ID=("ND" "D1" "D2" "D3") # 1100x, 125x, 250x, and 500x 
...
for idx_DP in ${!DP_ID[@]};do
	...
	if [[ $DP == *"ND"* ]];then
		...
		PU=$DIR/4.Analysis_Mosaic/0.PU/1.DF_A/parse_mx_wc2/0.by_var_type ## MUST BE WITHOUT CLIP (version 2)
		...
		PU=$DIR/4.Analysis_Mosaic/0.PU/3.DS_A/parse_mx_wc/0.by_var_type ## MUST BE WITHOUT CLIP (version 2)
	fi
	CASE_LIST=(${CASE_BAM// / })
		...
		for idx in ${!CTRL_LIST[@]};do
			...
			for mut in ${Mutant[*]};do
				...
				for tool_n in ${TOOL[*]};do
					...
					DataPath=$DIR/4.Analysis_Mosaic/$tool_n/$path_tag/mx
					STEP1=$FINAL/interim/1.conc/$path_tag/mx
					...
					log=$out/$tool_n/2.rescue_alt3/$path_tag
		        	       	...
		        	       	STEP2=$FINAL/interim/2.rescue_alt3/$path_tag/mx
					...
					DATA=$DIR/4.Analysis_Mosaic/$tool_n/$path_tag/mx
					STEP3=$FINAL/interim/3.extract_data/$path_tag/mx
